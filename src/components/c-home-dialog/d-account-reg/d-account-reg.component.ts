...
export class DAccountRegComponent implements OnInit, OnDestroy {
  ...
  async ngOnInit(): Promise<void> {
    ...
    this.accountTypeList$ = this.dialogService
      .getAccountTypeList()
      .valueChanges.pipe(
        map(({ data }) => {
    ...
    this.appSubscription.push(
      this.accountRegForm.get('GIDFull')!.valueChanges.subscribe((event) => {
        this.accountRegForm
          .get('GIDFull')!
  ...
  gidFullErr(): string | null {
    if (this.accountRegForm.get('GIDFull')!.hasError('required'))
      return '空白は禁止！';
    else if (this.accountRegForm.get('GIDFull')!.hasError('isCharValid'))
      return '特殊文字は使用できません！';
    return null;
  }

  fullNameErr(): string | null {
    if (this.accountRegForm.get('FullName')!.hasError('required'))
      return '空白は禁止！';
    else if (this.accountRegForm.get('FullName')!.hasError('isCharValid'))
      return '特殊文字は使用できません！';
    return null;
  }
  accLvlErrorMsg(): string | null {
    if (this.accountRegForm.get('AccLvl')?.hasError('required'))
      return '空白は禁止！';
    return null;
  }
  machineModelErr(): string | null {
    if (this.accountRegForm.get('machineModel')!.hasError('required'))
      return '空白は禁止！';
    else if (this.accountRegForm.get('machineModel')!.hasError('isCharValid'))
      return '特殊文字は使用できません！';
    return null;
  }

  passwordAErrorMsg(): string | null {
    if (this.accountRegForm.get('pass')!.hasError('required'))
      return '空白は禁止！';
    else if (this.accountRegForm.get('pass')!.hasError('minlength'))
      return '5文字以上必要です！';
    else if (this.accountRegForm.get('pass')!.hasError('isCharValid'))
      return '特殊文字は使用できません！';
    else if (this.accountRegForm.get('pass')!.hasError('whitespace'))
      return 'スペースは許可されていません！';
    return null;
  }
  passwordBErrorMsg(): string | null {
    if (this.accountRegForm.get('passB')!.hasError('required'))
      return '空白は禁止！';
    else if (this.accountRegForm.get('passB')!.hasError('minlength'))
      return '5文字以上必要です！';
    else if (this.accountRegForm.get('passB')!.hasError('isCharValid'))
      return '特殊文字は使用できません！';
    else if (this.accountRegForm.get('passB')!.hasError('whitespace'))
      return 'スペースは許可されていません！';
    else if (this.accountRegForm.get('passB')!.hasError('matching'))
